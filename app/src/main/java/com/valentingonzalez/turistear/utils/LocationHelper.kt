...
import android.location.Location
 ...
 object LocationHelper {
    ...
    fun WSG84toECEF(location: Location): FloatArray {
        val radLat = Math.toRadians(location.getLatitude())
        val radLon = Math.toRadians(location.getLongitude())
        ...
        val x = ((N + location.getAltitude()) * clat * clon).toFloat()
        val y = ((N + location.getAltitude()) * clat * slon).toFloat()
        val z = ((N * (1.0 - WGS84_E2) + location.getAltitude()) * slat).toFloat()
